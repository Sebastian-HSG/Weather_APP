...
# ______________________________________________________________________________
# 3. Define functions
# What we do with if statements here should be defined as functions that can be called only with their name later

def func1(lat, lon):
    url = "https://api.openweathermap.org/data/2.5/onecall?lat=%s&lon=%s&appid=%s&units=metric" % (lat, lon, api_key)
    # Cookie points for keeping the url out of the function and up in 2.
    response = requests.get(url)
    data = json.loads(response.text)
    # For now select only current weather from pulled data
    current = data["current"]
    return current
    # Todo: return only value specified in 4. (e.g. temperature)

"""
# Ignore func2 for now
def func2():
    # Layer name
    layer = "clouds_new"
...
while True:
    ...
    if task == '1':
        ...
        #Print output here for now, this will later move to 5.
